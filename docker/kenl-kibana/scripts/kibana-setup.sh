...
# *********** Setting Variables ***************
KIBANA="kenl-kibana:5601"
TIME_FIELD="@timestamp"
#DEFAULT_INDEX="logs-endpoint-winevent-sysmon-*"
DEFAULT_INDEX="logs-endpoint-kenl-*"
DIR=/usr/share/kibana/dashboards

# *********** Setting Index Pattern Array ***************
declare -a index_patterns=("logs-endpoint-*" "logs-*" "logs-endpoint-winevent-sysmon-*" "logs-endpoint-winevent-security-*" "logs-endpoint-winevent-system-*" "logs-endpoint-winevent-application-*" "logs-endpoint-winevent-wmiactivity-*" "logs-endpoint-winevent-powershell-*" "mitre-attack-*" "logs-endpoint-kenl-*")

# *********** Waiting for Kibana to be available ***************
echo "[++] Checking to see if kibana is up..."
until curl -v kenl-kibana:5601 -o /dev/null; do
    sleep 1
done
